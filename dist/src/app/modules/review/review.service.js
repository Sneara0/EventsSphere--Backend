...
const createReviewIntoDB = async (userId, payload) => {
    ...
    if (!isBooked) {
        throw new AppError(httpStatus.FORBIDDEN, "You cannot review an event you haven't booked or paid for.");
    }
 ...
 */
const getEventReviewsFromDB = async (eventId) => {
 ...
 */
const updateReviewInDB = async (userId, reviewId, payload) => {
 ...
 * 4. Delete a review
 */
const deleteReviewFromDB = async (userId, reviewId) => {
    const isReviewExists = await prisma.review.findUnique({
        where: { id: reviewId },
    });
    if (!isReviewExists) {
