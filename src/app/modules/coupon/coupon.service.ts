...
const createCouponIntoDB = async (payload: ICouponCreatePayload) => {
  ...
  if (isExists) throw new AppError(httpStatus.BAD_REQUEST, "Coupon code already exists");
...
const validateAndCalculateDiscount = async (code: string, eventId: string, originalAmount: number) => {
  const coupon = await prisma.coupon.findUnique({ where: { code: code.toUpperCase() } });

  if (!coupon) throw new AppError(httpStatus.NOT_FOUND, "Invalid coupon code");
  if (new Date() > new Date(coupon.expiryDate)) throw new AppError(httpStatus.BAD_REQUEST, "Coupon expired");
  if (coupon.usedCount >= coupon.usageLimit) throw new AppError(httpStatus.BAD_REQUEST, "Usage limit reached");
  if (coupon.eventId && coupon.eventId !== eventId) throw new AppError(httpStatus.BAD_REQUEST, "Invalid for this event");
...
const updateCouponInDB = async (id: string, payload: ICouponUpdatePayload) => {
